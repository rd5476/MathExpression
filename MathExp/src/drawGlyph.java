    ...
    public void adjustCoord(){
        ...
        for(int i=0;i<op.size();i++) {
            switch (op.get(i)) {
                ...
                case PathIterator.SEG_CUBICTO:
                    newPath.curveTo(allx1.get(i)+Math.abs(minX)+offset,
                            ally1.get(i)+Math.abs(minY)+offsetY,
                            allx2.get(i)+Math.abs(minX)+offset,
                            ally2.get(i)+Math.abs(minY)+offsetY,
                            allx3.get(i)+Math.abs(minX)+offset,
                            ally3.get(i)+Math.abs(minY)+offsetY);
                    break;

                case PathIterator.SEG_LINETO:
                    newPath.lineTo(allx1.get(i)+Math.abs(minX)+offset,
                            (ally1.get(i)+Math.abs(minY))+offsetY);
                    break;

                case PathIterator.SEG_MOVETO:
                    newPath.moveTo(allx1.get(i)+Math.abs(minX)+offset,
                    		(ally1.get(i)+Math.abs(minY))+offsetY);
                    break;

                case PathIterator.SEG_QUADTO:
                    newPath.quadTo(allx1.get(i)+Math.abs(minX)+offset,
                            (ally1.get(i)+Math.abs(minY))+offsetY,
                            allx2.get(i)+Math.abs(minX)+offset,
                            ally2.get(i)+Math.abs(minY)+offsetY);
                    break;
...
//        BufferedImage image = new BufferedImage(2500, 2500, BufferedImage.TYPE_INT_BGR);

        try {
  //          Graphics2D graphic = image.createGraphics();
    //        File output = new File("output"+ch+unicode+".png");
            paint(graphics);  // actual drawing on your image
...
//            }
         ...
         /*   allx1.add(coords[0]+offset);
            ally1.add(coords[1]+offsetY);
            allx2.add(coords[2]+offset);
            ally2.add(coords[3]+offsetY);
            allx3.add(coords[4]+offset);
            ally3.add(coords[5]+offsetY);
*/
            //           System.out.println();
            iter.next();
        }
